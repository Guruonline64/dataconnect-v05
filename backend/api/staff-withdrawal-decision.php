...
try {
 ...
 if($decision==='reject'){
   ...
   notify_user((int)$w['user_id'],'Withdrawal rejected',$reason ?: 'Your withdrawal request was rejected.');
   audit_log((int)$staff['id'],'reject','withdrawal',$id,['reason'=>$reason]);
   $pdo->commit(); out(true,'Withdrawal rejected',['status'=>'rejected']);
 }
 $s=$pdo->prepare('SELECT balance FROM wallets WHERE user_id=? FOR UPDATE');$s->execute([$w['user_id']]);$wallet=$s->fetch();
 if(!$wallet || (float)$wallet['balance'] < (float)$w['amount']){$pdo->rollBack();out(false,'Insufficient available balance',[],409);}
 $ref='WD-'.date('YmdHis').'-'.bin2hex(random_bytes(4));
 ...
 notify_user((int)$w['user_id'],'Withdrawal approved','Your withdrawal request has been approved.');
